...
## ケース6: NG — Step 7 の確認がマイグレーション適用の確認に紛れて省略された（禁止パターン）

### 状況（悪い例）
- schema.ts を変更する改修で、Step 6（backend-review）完了後、そのまま「マイグレーションを適用してよいか」だけを確認し、Step 8（フロントエンド実装）に進んでしまった

```
（backend-review「問題なし」の直後）

Claude: schema.tsの変更をDBに反映するため、db:generateとdb:migrate:localを
        実行してよろしいですか？

User: お願い。

（マイグレーション適用 → そのままフロントエンド実装に着手）
```

### 何が問題か
- 「## バックエンド変更完了」形式の定型サマリーと「確認できたら実装を続けます。よろしいですか？」という Step 7 本来の確認が一度も提示されていない
- マイグレーション適用可否の確認は Step 5（バックエンド変更実装）に付随する実行系の確認であり、Step 7 の「変更内容そのものをユーザーと合意する」確認とは目的が異なる。前者で答えを得たことを後者の完了とみなしてはならない
- 修正方針: マイグレーション適用など Step 5 に付随する確認を終えたら、それとは別に Step 7 の定型フォーマットを出力し、明示的な OK を得てから Step 8 に進む
