...
name: backend-review
description: |
  バックエンドのコード変更が完了した直後に、設計品質（CLAUDE.md 基準）・レイヤー設計・単一責務の観点でチェックを行う。
...
バックエンド実装後に、設計品質（CLAUDE.md の「設計品質」の定義）・レイヤー設計（Presentation/Application/Domain/Infrastructure の4層DDD構成）・単一責務の違反パターンをチェックする。

**完了の基準は設計品質である。** 型チェック・テストが通っていても、「設計品質（CLAUDE.md 基準）」のチェックに違反がある場合は完了として報告しない。
...
### 設計品質（CLAUDE.md 基準・最優先）

Claude 自身が提案・実装した方式も例外なく対象とする。指摘を受けた箇所だけでなく、同じ原因を持つ周辺の設計も確認する。

**集約・VO が自分の値の正しさ（不変条件）を保証できているか**
- 集約が、自分では正しさを検証できない値を外から受け取っていないか
  - アンチパターン: 更新前の状態をユースケースが生データで取得し、集約のメソッドに引数で渡す（`aggregate.releasedTagIds(previousTagIds)` 等。どんな一覧でも通ってしまう）
  - アンチパターン: 他集約の属性（名前等）のコピーを保持する（ID 参照で足りる場合）
  - 正しいパターン: リポジトリが既存の集約を復元し、集約自身のメソッドで変更を適用して差分を求める
- リポジトリの `reconstruct` 呼び出しで、実データと異なる状態（`tagIdList: []` のような空配列の仮置き等）を渡していないか
- 状態遷移メソッド（更新・復元・完全削除等）が、前提条件（削除済み／未削除等）を集約自身で検証しているか（リポジトリの取得条件だけに頼っていないか）

**ルールが正しい層にあるか**
- ドメインルールが Usecase・Repository・SQL の中にしか存在しない状態になっていないか
  - アンチパターン: 「未使用のタグを削除する」の判定を、リポジトリの `NOT EXISTS` だけで行っている
  - D1（トランザクション不可）の都合で SQL 側に最終確認が必要な場合でも、判定そのものはドメインサービス・集約で行い、SQL は最終防衛に限定しているか（前例: タイトル一意性 = `RankingTitleUniquenessDomainService` の判定 + DB の一意インデックス）
  - repository interface の JSDoc に、そのメソッドが実行するドメイン上の副作用（削除等）が明記されているか
- ドメインサービスが生データから集約を再構築していないか（再構築はリポジトリの責務。repository interface は集約を返す）
- 集約同士が型・クラスを直接 import していないか（複数集約にまたがる処理はドメインサービスで組み立てる）

**合意済みの方針・前例と一致しているか**
- `docs/[機能名]/plan.md` の決定事項、および CLAUDE.md の設計指針と矛盾していないか
- 既存コードに処理を積み足す場合、積み足す先の既存コード自体が合意済みの方針からずれていないか（ずれていれば、その上に積み足さず報告する）
  - 例: ランキング更新は「既存集約を復元 → 集約のメソッドで適用 → 最終集約を保存」の方針だが、既存の更新ユースケースが `RankingAggregate.create` で作り直していた
...
4. 「設計品質（CLAUDE.md 基準・最優先）」を最初に照合する。違反がある場合は、型チェック・テストの結果にかかわらず完了として扱わない
5. 残りの各チェック項目を照合する
6. 以下の形式で報告する
