...
React + Hono + RPC を用いたフロントエンド・バックエンド統合アプリケーション（認証付き）
...
本リポジトリはテンプレートリポジトリを起点に構築したアプリケーションであり、テンプレートそのものではない。
設計品質を最優先とする。ここでの「設計品質」とは、以下の正しさを指す。

【要素単体の正しさ】

* 責務が適切に分離されていること
* 値オブジェクト・エンティティ・集約が、それぞれ担当する値の正しさ（不変条件）を担保する仕組みになっていること
* 概念の異なるものを誤って共通化していないこと

【要素間の関係・配置の正しさ】

* レイヤーの依存方向が一方向であること（presentation → application → domain）。
  domain は infrastructure や framework（Hono / Drizzle）に依存せず、repository は interface を domain に置き infrastructure が実装する（依存性逆転）
* ドメイン層が技術詳細（Context・DB・環境変数取得等）を知らず、ドメインルールが controller / usecase に流出していないこと
* フォルダ構成が「境界（context）× レイヤー」を一貫して表現し、新規要素の置き場所が一意に定まること。
  domain/shared に置くのは複数コンテキストで真に共有される概念だけとし、安易に共通化しないこと

「分かりやすさ」とは、依存や記述量を減らすことではない。上記の正しさを指す。
依存の少なさ・記述量の少なさを「分かりやすさ」「シンプルさ」の名目で優先してはならない。
...
* 説明は前提知識のない人が読んでも分かる平易な言葉で書くこと
  * 結論を先に一言で書き、理由はその後に短く添える
  * 1文を短くし、専門用語や曖昧な言葉（「共通の場所」等）はその場で具体的に言い換える
...
* 指示なくビルドを実行しないこと
* devサーバー起動の許可は、その上でデータを作成してよい許可ではない。動作確認・デバッグ目的であっても、DBへの書き込み（ユーザー登録・レコード作成/更新/削除等）を行う前には必ず一言断ってから実行すること。ローカル環境・ローカルDBであっても例外なし
