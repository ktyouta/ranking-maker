...
import { BulkRestoreMyRankingRepository } from "../../../../src/infrastructure/my-ranking/repository/bulk-restore-my-ranking.repository";

describe("BulkRestoreMyRankingRepository", () => {

    it("findRankings→restore→restoreRankings: ランキングごとに削除済みのタグ付けを集約に含め、復元状態をタグ付けに反映すること", async () => {
        ...
        await db.insert(rankingMaster).values(rankingIds.map((id) => ({ id, userId, title: `test-ranking-${id}`, publicStatus: publicStatusId, deleteFlg: true, createdAt: now, updatedAt: now })));
        await db.insert(tagMaster).values({ id: tagId, userId, name: "共有タグ", createdAt: now, updatedAt: now });
        await db.insert(rankingTagMaster).values(rankingIds.map((rankingId, index) => ({ id: rankingTagIds[index], rankingId, tagId, userId, deleteFlg: true, createdAt: now, updatedAt: now })));
        ...
        rankings.forEach((ranking) => ranking.restore());
        await repository.restoreRankings(rankings);
